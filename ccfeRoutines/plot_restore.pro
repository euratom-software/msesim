function plot_restore,system,noreset=noreset,_extra=ex
  ...
  IF N_TAGS(system) EQ 6 THEN BEGIN
    ...
    IF found_tag EQ 6 THEN BEGIN
      ...
      if not keyword_set(noreset) then begin
          ...
          ENDIF ELSE print,'Can not restore color table-> needs to be a structure'
      endif
    ...
    ENDELSE
  ENDIF ELSE BEGIN
    print,'Argument needs to be a structure returned by plot_init'
  ENDELSE
end
