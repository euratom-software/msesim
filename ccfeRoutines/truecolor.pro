...
function truecolor, arg, arg2, arg3,  _extra=extra,       $
  ...
  if (n_elements(col_list) eq 0) then begin
    ncol=141
    col_list=replicate({name:'', hex:'', r:0B, g:0B, b:0B, rgb:0L}, ncol)
    tmp=strarr(ncol)
    ...
    rgb=col_list.hex
    col_list.hex=strmid(rgb, 4, 2)+strmid(rgb, 2, 2)+strmid(rgb, 0, 2)
    rgb=col_list.rgb
    reads, col_list.hex, rgb, format='(z)'
    col_list.rgb=rgb
    
    col_list.b=rgb/65536
    col_list.r=rgb mod 256
    col_list.g=(rgb/256) mod 256

    col_list=temporary(shift(col_list, 1))
  endif

  if keyword_set(setcolortable) or (!d.name eq 'PS') then begin
    tvlct, r, g, b, /get
    if ~array_equal([r,g,b], [col_list.r,col_list.g,col_list.b]) then tvlct, col_list.r, col_list.g, col_list.b
  endif

  if keyword_set(showlist) then return, col_list(1:*).name
  ...
  for i=0, n_elements(name)-1 do begin
    pos=where(strpos(col_list.name, name[i]) eq 0, flag)
  ...
  if (!d.name eq 'X') or (!d.name eq 'WIN') then begin
    ...
    if (decompsetting) then return, col_list[rgb].rgb
  endif
